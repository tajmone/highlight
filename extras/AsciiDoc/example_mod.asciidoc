...
:revdate: {modDate}
:version-label: mod
:revremark: Last edited: 2019-03-30
//
:toclevels: 5
:toc: left
:experimental: true
...
link:./highlight_mod.css[The custom CSS stylesheet] defines a default theme plus some dedicated themes targetting specific syntaxes.
...
Text enclosed withing single or double `&num;` symbols will be wrapped in the `<mark>` tag (See
link:https://asciidoctor.org/docs/user-manual/#custom-styling-with-attributes[Asciidoctor Manual §19.5^]), which I've customized to mimick a smeared flourescent marker.
...
In the link:./sass[`/sass/`] folder you'll find the commented SCSS sources used by this document, and a useful Sass template for quickly generating Highlight themes targetting specific syntaxes.
...
This ensures that custom langDefs inside `HIGHLIGHT_DATADIR` will always override the predefined ones of Highlight installation folder, even on Windows where the path of the `HIGHLIGHT_DATADIR` environment variable would be searched _after_ Highlight installation folder (unlike on Linux and Mac).
